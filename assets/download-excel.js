...
// Function to handle data from API for download
window.exportToExcelWithData = async function(reportData) {
    try {
        console.log('Memulai proses export Excel dengan data dari API:', reportData);
        ...
        // Data form di sebelah kanan (H2:K8)
        worksheet.getCell('H2').value = reportData.lokasi || '';
        worksheet.getCell('H3').value = reportData.tiang || '';
        worksheet.getCell('H4').value = reportData.pon || '';
        worksheet.getCell('H5').value = reportData.ms || '';
        worksheet.getCell('H6').value = reportData.odp || '';
        worksheet.getCell('H7').value = reportData.redaman_in || '';
        worksheet.getCell('H8').value = reportData.redaman_out || '';
        
        // Keterangan di G10:K18 (merged cell)
        worksheet.getCell('G10').value = reportData.notes || '';
        
        console.log('Data form berhasil ditempatkan');
        
        // Kumpulkan data gambar
        console.log('Mengumpulkan data gambar...');
        const imageData = reportData.images || [];
        console.log('Data gambar:', imageData);
        ...
        for (let i = 0; i < imageData.length; i++) {
            ...
            if (image && image.image_url) {
                try {
                    ...
                    // Dapatkan dimensi asli gambar
                    const imageDimensions = await getImageDimensionsFromBlob(blob);
                    ...
                    const imageId = workbook.addImage({
                        buffer: bytes,
                        extension: getImageExtensionFromUrl(image.image_url)
                    });
